...
//        exit();
        if ($request->isMethod('post')){
            ...
            unset($max_pwd['role']);
            //验证
            if (\Auth::guard('frontd')->attempt($max_pwd)) {


                //手机号登录
                if ($request->has('phone')) {
                    if ($mg_id = Driver::select('driver_id', 'driver_name','token','title_image')->where('phone', $max_pwd['phone'])->limit(1)->get()->toArray()) {


                        $token =  encrypt_pass($mg_id[0]['token'],$max_pwd['password']);
                        $token_time = date('Y-m-d H:i:s', time()+3600*2);

                        Driver::where('driver_id',$mg_id[0]['driver_id'])->update(['token_time'=>$token_time]);

                        return ['status' => 200,
                            'token' => $mg_id[0]['token'],
                            'mi_token'=>$token,
                            'driver_id' => $mg_id[0]['driver_id'],
                            'driver_name' => $mg_id[0]['driver_name'],
                ...
                //邮箱登录
                    ...
                    if ($mg_id = Driver::select('driver_id', 'driver_name','token','title_image' )->where('email', $max_pwd['email'])->limit(1)->get()->toArray()) {
    ...
    public function sendSMS(Request $request){
        ...
        if ($request->isMethod('post')) {
            ...
            if ($mg_id = Driver::select('driver_id')->where('phone', $phone)->limit(1)->get()->toArray()) {
    ...
    public function checkCode(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if ($temp_phone->code != $phone_code || time() > strtotime($temp_phone->deadline)) {
                ...
                $mg_id = Driver::select('driver_id', 'driver_name','token','title_image')->where('phone', $phone)->limit(1)->get()->toArray();
    ...
    {
        \Auth::guard('frontd')->logout();//清除session
        return ['status' => 200];
    }
    /**
     * 司机列表
    ...
    public function create(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if(phone_yz($formData['phone'],'driver_id')){
